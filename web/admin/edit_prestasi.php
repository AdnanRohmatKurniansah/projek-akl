...
<html>
  ...
  <body>
      ...
      <center>
      <form method="POST" action="proses_editprestasi.php" enctype="multipart/form-data" >
      <section class="base">
        <!-- menampung nilai id produk yang akan di edit -->
        <input name="id" value="<?php echo $data['id']; ?>"  hidden />
        <div>
          <label>Foto</label>
          <img src="imgPrestasi/<?php echo $data['foto']; ?>" style="width: 80px;float: left;margin-bottom: 5px;">
          <input type="file" name="foto" />
          <i style="float: left;font-size: 11px;color: red">Abaikan jika tidak merubah foto</i>
        </div>
        <div>
          <label>Nama Prestasi</label>
          <textarea style="width: 100%; height: 100px;" name="nama_prestasi"><?php echo $data['nama_prestasi']; ?></textarea>
        </div>
        <div>
          <label>Juara</label>
         <input type="text" name="juara" required="" value="<?php echo $data['juara']; ?>"/>
        </div>
        <div>
          <label>Tahun</label>
