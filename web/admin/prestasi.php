...
<div class="content-page">
    <!-- Start content -->
    <div class="content">    
    <center><h1>Daftar Prestasi</h1><center>
  <div class="add">
  <center><a href="tambah_prestasi.php">+ &nbsp; Tambah Data</a><center></div>
    <br>
    <form method="POST" class="d-flex" role="search" style="width: 50%; ali">
        <input  type="text"  name="t_cari" value="<?= @$_POST['t_cari'] ?>"  class="form-control me-2" placeholder="Masukkan Nama Prestasi">
    ...
    if (isset($_POST['b_cari'])) {
      ...
      $reset       = "SELECT * FROM prestasi WHERE nama_prestasi like '%$keyword%' or juara like '%$keyword%' or
      tahun like '%$keyword%' order by nama_prestasi asc";
    } else {
      $reset = "SELECT * FROM prestasi order by nama_prestasi asc";
    }
    $prestasi = mysqli_query($koneksi, $reset);
    while ($data = mysqli_fetch_array($prestasi)) :
    ?>
      
       <tr>
          <td><?php echo $no; ?></td>
          <td style="text-align: center;"><img src="imgPrestasi/<?php echo $data['foto']; ?>" style="width: 100px; height: 100px;"></td>
          ...
          <td>
              <a href="edit_prestasi.php?id=<?php echo $data['id']; ?>" style="background-color: green; color: #fff;
          padding: 10px; text-decoration: none; font-size: 12px;">Edit</a> |
              <a href="proses_hapusprestasi.php?id=<?php echo $data['id']; ?>" onclick="return confirm('Anda yakin akan menghapus data ini?')" style="background-color: red; color: #fff;
